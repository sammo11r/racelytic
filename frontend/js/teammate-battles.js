...
function renderBattleSeasons() {
  ...
  return `<div class="battle-season-list">${seasons.map(year=>{const season=races.filter(race=>race.year===year),race=battleScore(season,'firstPosition','secondPosition'),qualifying=battleScore(season,'firstQualifying','secondQualifying'),firstPoints=pointsTotal(season,'firstPoints'),secondPoints=pointsTotal(season,'secondPoints'),teams=[...new Set(season.map(item=>item.constructorName).filter(Boolean))].join(', ');return `<article class="battle-season-card"><div><span>${year}</span><h3>${esc(teams)}</h3><small>${season.length} shared race${season.length===1?'':'s'}</small></div><dl><div><dt>Race H2H</dt><dd>${race.first}–${race.second}</dd></div><div><dt>Qualifying H2H</dt><dd>${qualifying.first}–${qualifying.second}</dd></div><div><dt>Points</dt><dd>${fmtNumber(firstPoints)}–${fmtNumber(secondPoints)}</dd></div></dl><a href="/season?year=${year}">View season →</a></article>`;}).join('')}</div>`;
}

function renderBattleRaces() {
  const races=battleData.sharedRaces,[first,second]=battleData.drivers;
  return `<div class="table-wrap"><table><thead><tr><th>Race</th><th>Team</th><th>${esc(first.name)}</th><th>${esc(second.name)}</th><th>Qualifying</th><th>Points</th></tr></thead><tbody>${races.map(race=>`<tr><td><a href="/race?id=${encodeURIComponent(race.raceId)}">${esc(race.year)} ${esc(race.officialName)}</a><small>Round ${esc(race.round)}</small></td><td>${esc(race.constructorName||'—')}</td><td>${esc(positionLabel(race.firstPositionText,race.firstPosition))}</td><td>${esc(positionLabel(race.secondPositionText,race.secondPosition))}</td><td>${race.firstQualifying||'—'}–${race.secondQualifying||'—'}</td><td>${fmtNumber(race.firstPoints)}–${fmtNumber(race.secondPoints)}</td></tr>`).join('')}</tbody></table></div>`;
}
...
function renderBattle() {
  ...
  document.getElementById('battle-content').innerHTML=`<section class="comparison-driver-heads"><a href="/driver?id=${encodeURIComponent(first.id)}"><span>${esc(first.nationalityCountryId||'')}</span><h2>${esc(first.name)}</h2></a><div>${races.length} SHARED RACES<br>${Math.min(...years)}–${Math.max(...years)}</div><a href="/driver?id=${encodeURIComponent(second.id)}"><span>${esc(second.nationalityCountryId||'')}</span><h2>${esc(second.name)}</h2></a></section><section class="analysis-visualization-menu"><div class="analysis-visualization-heading"><div class="eyebrow">BATTLE VIEW</div><strong>Compare the pairing</strong></div><div class="analysis-visualization-tabs comparison-tabs" role="tablist"><button type="button" class="active" role="tab" aria-selected="true" data-battle-view="overview"><span>Overview</span><small>Head-to-head</small></button><button type="button" role="tab" aria-selected="false" data-battle-view="seasons"><span>Shared seasons</span><small>Year by year</small></button><button type="button" role="tab" aria-selected="false" data-battle-view="races"><span>Shared races</span><small>Full results</small></button></div></section><div class="comparison-workspace"><section data-battle-panel="overview">${renderBattleOverview()}</section><section data-battle-panel="seasons" hidden>${renderBattleSeasons()}</section><section data-battle-panel="races" hidden>${renderBattleRaces()}</section></div>`;
