...
async function populateRaces(){const token=++racePopulationToken,year=document.getElementById('race-analysis-year').value,races=analysisRaces.filter(race=>String(race.year)===year).sort((a,b)=>a.round-b.round),selector=document.getElementById('race-analysis-race');if(!juniorRaceAnalysis){selector.innerHTML=races.map(race=>`<option value="${esc(race.id)}">R${esc(race.round)} · ${esc(displayRaceName(race))}</option>`).join('');return loadRaceAnalysis();}selector.innerHTML='<option>Loading race sessions…</option>';try{const weekends=await Promise.all(races.map(async race=>{const data=raceAnalysisDetails.get(String(race.id))||await getJSON(`/api/races/${encodeURIComponent(race.id)}`);raceAnalysisDetails.set(String(race.id),data);return{race,data};}));if(token!==racePopulationToken)return;const options=weekends.flatMap(({race,data})=>data.sessions.filter(session=>session.isRace&&!session.cancelled&&session.results?.length).map(session=>`<option value="${esc(race.id)}::${esc(session.id)}">R${esc(race.round)} · ${esc(displayRaceName(race))} · ${esc(session.displayName||session.name)}</option>`));selector.innerHTML=options.join('');if(options.length)loadRaceAnalysis();else setError('race-flow-chart','No completed race sessions are available for this season.');}catch(error){if(token===racePopulationToken)setError('race-flow-chart',error.message);}}
