...
async function loadF2Constructor() {
  ...
  const id = resourceId('constructor');
  ...
  try {
    ...
    document.getElementById('f2-constructor-drivers').innerHTML = data.drivers.map(driver => `<a class="constructor-driver-card" href="/f2/drivers/${encodeURIComponent(driver.driverId)}"><div class="constructor-driver-years">${driver.firstYear === driver.lastYear ? esc(driver.firstYear) : `${esc(driver.firstYear)}–${esc(driver.lastYear)}`}</div><strong>${esc(driver.driverName)}</strong><span>${fmtNumber(driver.starts)} starts · ${fmtNumber(driver.points)} points</span><div class="constructor-driver-record"><small>${fmtNumber(driver.wins)} wins</small><small>${fmtNumber(driver.podiums)} podiums</small><small>${fmtNumber(driver.seasons)} seasons</small></div></a>`).join('') || '<div class="empty-state">No driver history available.</div>';
    let resultPage = 1;
    const renderResults = () => {
      const paged = pageItems(data.results, resultPage, 30);
      resultPage = paged.page;
      document.getElementById('f2-constructor-results').innerHTML = paged.items.length ? `<table><thead><tr><th>Season</th><th>Weekend</th><th>Session</th><th>Driver</th><th>Finish</th><th>Points</th></tr></thead><tbody>${paged.items.map(result => `<tr><td><a href="${resourceUrl('season',result.year,{base:'/f2'})}">${esc(result.year)}</a></td><td><a href="${resourceUrl('race',result.raceId,{base:'/f2',label:result.raceName})}">${esc(result.raceName)}</a><small>${esc(fmtDate(result.date))}</small></td><td>${esc(result.sessionName)}</td><td><a href="${resourceUrl('driver',result.driverId,{base:'/f2'})}">${esc(result.driverName)}</a></td><td><span class="finish-position${Number(result.positionNumber) <= 3 && Number(result.positionNumber) > 0 ? ' podium' : ''}">${esc(result.positionNumber || result.status || '—')}</span></td><td class="result-points-total">${fmtNumber(result.points)}</td></tr>`).join('')}</tbody></table>` : '<div class="empty-state">No race results available.</div>';
