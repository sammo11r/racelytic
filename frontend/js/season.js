(() => {
    ...
    function resultClass(result) {
        ...
        if (/\b(?:DSQ|DQ|DISQ|DISQUALIFIED|EXC)\b/.test(text)) return 'result-disqualified';
        if (!result.position || /RET|DNS|DNQ|WD|NC/.test(text)) return 'result-retired';
    ...
    function constructorResultClass(result) {
        if (!result) return 'result-empty';
        ...
        if (Number(result.points || 0) + Number(result.sprintPoints || 0) > 0) return 'result-points';
    ...
    function renderSummary(summary) {
        ...
        const labels = {
            first: completed ? 'World champion' : 'Championship leader',
            second: completed ? 'Runner-up' : 'Second place',
            third: 'Third place',
            constructor: completed ? 'Constructors’ champion' : 'Leading constructor'
        };

        for (const [position, label] of Object.entries(labels)) {
            const element = get(`season-${position}-label`);
        ...
        const constructor =
            get('season-constructor');

        if (constructor) {
            constructor.textContent =
                summary.constructorLeader?.name || '—';
        }
    }
    ...
    function renderConstructorTable(
        ...
        for (const constructor of constructors) {
            ...
            for (const race of races) {
                ...
                rows += `
                    <td
                        class="race-point constructor-points ${constructorResultClass(result)}"
                    >
    ...
    async function renderSeasonMap(races) {
        ...
        try {
            ...
            const positionMarkers = () => {
                ...
                stops.attr('transform', race =>
                    `${markerPosition(race)} scale(${inverseScale})`
                );
            };
            ...
            const draw = () => {
                ...
                markerPosition = race =>
                    `translate(${projection([Number(race.longitude), Number(race.latitude)])})`;
                ...
                viewport.append('g').selectAll('path').data(countries.features).join('path')
                    .attr('class', 'map-country').attr('d', path);
                viewport.append('path').datum({ type: 'LineString', coordinates })
            ...
            container.querySelector('[data-map-action="reset"]')?.addEventListener('click', () => {
                svg.transition().duration(220).call(zoom.transform, window.d3.zoomIdentity);
            });
