...
function updateStatus() {
  ...
  if (!completion.hidden) { document.getElementById('quiz-completion-label').textContent = quizGaveUp ? 'Answers revealed' : 'Quiz complete'; document.getElementById('quiz-completion-title').textContent = quizGaveUp ? `${selectedYear} calendar completed` : 'Perfect score'; document.getElementById('quiz-completion-copy').textContent = quizGaveUp ? `You found ${names.length} winning drivers before revealing the remaining races.` : `You completed all ${total} race winners from the ${selectedYear} season.`; }
}
...
function renderSeasonPicker() { document.getElementById('quiz-season').innerHTML = availableYears.map(year => `<option value="${year}"${year === selectedYear ? ' selected' : ''}>${year}</option>`).join(''); }
...
document.getElementById('season-winner-guess-form').addEventListener('submit', async event => {
  ...
  try { const response = await fetch(`/api/games/season-race-winners/guess${seriesQuery('?')}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ year: selectedYear, guess }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Unable to check that guess.');
    if (!result.correct) { feedback.textContent = `${guess} did not win a race in ${selectedYear}.`; feedback.className = 'is-incorrect'; }
...
document.getElementById('quiz-reset').addEventListener('click', () => confirmAction('Reset this season?', `Every answer found for ${selectedYear} will be removed.`, 'Reset quiz', resetQuiz));
document.getElementById('quiz-reveal').addEventListener('click', () => confirmAction('Reveal every race winner?', `All remaining ${selectedYear} winners will be shown and this attempt will be marked as revealed.`, 'Reveal answers', () => revealQuiz().catch(error => { document.getElementById('quiz-feedback').textContent = error.message; })));
