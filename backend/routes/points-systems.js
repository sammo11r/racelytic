...
const SELECT_FIELDS = `
    ps.id, ps.user_id AS userId, u.display_name AS ownerName, ps.name, ps.series,
    ...
    try {
        ...
        await pool.query(`
            INSERT INTO app_points_systems (
                id, user_id, name, series, race_points, sprint_points, qualifying_points, pole_bonus,
                ...
                sprint_counts_toward_round, visibility, tie_breaker, extended_multiplier, le_mans_multiplier
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [
            id, req.user.id, system.name, system.series, JSON.stringify(system.racePoints), JSON.stringify(system.sprintPoints),
            ...
            system.extendedMultiplier, system.leMansMultiplier
        ]);
...
router.put('/api/points-systems/:id', requireUser, async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            UPDATE app_points_systems SET name = ?, series = ?, race_points = ?, sprint_points = ?, qualifying_points = ?, pole_bonus = ?,
                fastest_lap_bonus = ?, fastest_lap_max_position = ?, count_best_rounds = ?,
                best_first_rounds = ?, first_rounds_window = ?, best_last_rounds = ?, last_rounds_window = ?,
                sprint_counts_toward_round = ?, visibility = ?, tie_breaker = ?,
                extended_multiplier = ?, le_mans_multiplier = ?
            WHERE id = ? AND user_id = ?
        `, [
            system.name, system.series, JSON.stringify(system.racePoints), JSON.stringify(system.sprintPoints), JSON.stringify(system.qualifyingPoints), system.poleBonus,
