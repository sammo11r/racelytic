...
function readF3Drivers() {
  return new Promise((resolve, reject) => {
    const rows = [];
    fs.createReadStream(path.join(__dirname, '../data/f3db-drivers.csv'))
      .pipe(csv())
      .on('data', row => rows.push(row))
      .on('end', () => resolve(rows))
      .on('error', reject);
...
function readF3Constructors() {
  return new Promise((resolve, reject) => {
    const rows = [];
    fs.createReadStream(path.join(__dirname, '../data/f3db-constructors.csv'))
      .pipe(csv())
      .on('data', row => rows.push(row))
      .on('end', () => resolve(rows))
      .on('error', reject);
...
test('every Formula 3 driver has a valid nationality and flag asset', async () => {
  const drivers = await readF3Drivers();
  const flagDirectory = path.join(__dirname, '../frontend/assets/flags');
  const invalid = drivers
    .filter(driver => !/^[a-z]{2}$/.test(String(driver.countryCode || '')) ||
      !fs.existsSync(path.join(flagDirectory, `${driver.countryCode}.svg`)))
    .map(driver => `${driver.name} (${driver.countryCode || 'missing'})`);
...
test('every Formula 3 team has a valid nationality and flag asset', async () => {
  const constructors = await readF3Constructors();
  const flagDirectory = path.join(__dirname, '../frontend/assets/flags');
  const invalid = constructors
    .filter(constructor => !/^[a-z]{2}$/.test(String(constructor.countryCode || '')) ||
      !fs.existsSync(path.join(flagDirectory, `${constructor.countryCode}.svg`)))
