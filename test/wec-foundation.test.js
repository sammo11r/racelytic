...
const { championshipClassIds, championshipPdfMatches } = require('../scripts/collect-wec-data');
...
test('WEC has a distinct endurance identity and public namespace', () => {
    ...
    assert.equal(resourcePath('wec', 'carModel', 'ferrari-499p'), '/wec/car-models/ferrari-499p');
...
test('WEC uses a distinct endurance-green accent rather than the F2 blue palette', () => {
    ...
    assert.match(favicon, /fill="#087f5b"/);
});
...
test('WEC foundation covers every completed championship from 2012 through 2025', async () => {
    const dataset = await loadWecFoundation();
    assert.deepEqual(validateWecFoundation(dataset), { seasons: 13, events: 101, circuits: 15, classes: 54 });
    assert.deepEqual(dataset.seasons.map(season => season.id), [
        'wec-2012', 'wec-2013', 'wec-2014', 'wec-2015', 'wec-2016', 'wec-2017',
        'wec-2018-2019', 'wec-2019-2020', 'wec-2021', 'wec-2022', 'wec-2023', 'wec-2024', 'wec-2025'
    ]);
...
test('WEC classifications and standings link to archive profiles', () => {
    ...
    assert.match(server, /\['drivers', 'teams', 'manufacturers', 'car-models', 'entries'\]/);
});
...
test('WEC season page presents a focused championship header card', () => {
    ...
    assert.match(route, /results\.classPosition IS NOT NULL/);
...
test('WEC landing and race sessions remain interactive without losing entry context', () => {
    const home = fs.readFileSync(path.join(root, 'frontend/wec.html'), 'utf8');
    const script = fs.readFileSync(path.join(root, 'frontend/js/wec.js'), 'utf8');
    assert.match(home, /id="wec-home-event"/);
    assert.match(home, /id="wec-home-leaders"/);
    assert.match(script, /getJSON\('\/api\/wec\/home'\)/);
    assert.doesNotMatch(script, /loadSeason\(seasons\[0\]\.year\)/);
    assert.match(script, /window\.history\.replaceState/);
